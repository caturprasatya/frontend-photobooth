...
const Capture = (props) => {
  ...
  if(state){
    ...
    objSize = findCropPxl(state.width,state.height,videoScaledWidth,videoScaledHeight);
  ...
  const takeSnap = (mode, actionFrom = 'function') => {
    if(mode === 'take' && actionFrom === 'layout') setLimitRetake(LIMIT_RETAKE); // reset limit, if user press button next
          ...
          <div>
            ...
            <div className="nextButton">
              ...
              <button type="button" className="btn btn-dark btn-lg" disabled={isNext} onClick={()=>takeSnap('take', 'layout')}>
                <h2><b>{imageBlob.length===0 ? "Start":"Next >>"}</b></h2>
              </button>
            </div>
          </div>
        </div>
